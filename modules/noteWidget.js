...
const zametkiModule = (() => {
    const zametkiArray = ['В эти лунные сутки можно смело назначать свидания, деловые встречи, а также проводить время с близкими людьми, занимаясь общими делами, приносящими удовольствие обоим партнерам.',
    ...
    'Сегодня ничто не сможет помешать осуществлению ваших замыслов! Сконцентрируйтесь на своих мечтах, и смело претворяйте их в жизнь.',
    ...
    return {
      getRandomAdvice: ()=> {
        const index = Math.floor(Math.random()*zametkiArray.length);
        return zametkiArray[index];
      }
    }
  })();
  ...
  //При нажатии на кнопку "показать еще" рандомно меняется заметка о лунном дне
    ...
    document.getElementById('text_zametki').innerText = zametkiModule.getRandomAdvice();
  });
